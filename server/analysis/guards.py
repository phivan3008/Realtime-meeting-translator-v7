...
from server.pipeline.asr import Piece, Transcriber, normalise_for_pattern
from server.wordlists import read_lines
...
def make_transcriber() -> Transcriber:
    ...
    return Transcriber(decoder=_NoDecoder(), prompt="")
...
def refuse(transcriber: Transcriber, piece: Piece,
    ...
    """
    if rule == "current":
        return transcriber._refuse(piece, utterance_ms)
    ...
    if piece.compression_ratio > transcriber.max_compression_ratio:
        return "repetition"
    if transcriber.hallucinations.is_invented(piece.text):
...
def near_miss(text: str, phrases=None) -> Optional[dict]:
    ...
    README in ``server/data``. This does not block
...
def _listed() -> list:
    """The block list, read once."""
    if not _LISTED:
        _LISTED.extend(read_lines("hallucinations.txt"))
    return _LISTED
