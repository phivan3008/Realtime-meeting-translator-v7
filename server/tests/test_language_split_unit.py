...
from server.pipeline import language_split  # noqa: E402
from server.pipeline.language_split import LanguageSplitter  # noqa: E402


def find_split(pcm, prober, **settings):
    """The cut alone; the verdict's reasons have tests of their own below."""
    return language_split.find_split(pcm, prober, **settings).split
...
class TestEveryRefusalSaysWhy:
    """A splitter that declined in silence is how the case it was built for
    once went by with no log line at all."""

    def test_one_language_is_counted(self):
        splitter = LanguageSplitter(FixedProber(decided("vi")))
        splitter.find(audio(6.0))
        assert splitter.stats.one_language == 1
        assert splitter.stats.checked == 1
        assert splitter.stats.probes == 2
        assert "one language" in splitter.stats.last

    def test_an_undecided_end_is_counted_with_its_margin(self):
        splitter = LanguageSplitter(FixedProber(UNDECIDED, decided("ja")))
        splitter.find(audio(6.0))
        assert splitter.stats.undecided == 1
        assert "0.05" in splitter.stats.last

    def test_too_short_is_counted_but_not_as_checked(self):
        splitter = LanguageSplitter(FixedProber(decided("vi")))
        splitter.find(audio(1.0))
        assert splitter.stats.too_short == 1
        assert splitter.stats.checked == 0

    def test_a_split_is_counted(self):
        splitter = LanguageSplitter(TimedProber("vi", "ja"))
        split = splitter.find(two_turns(3.0, 3.0))
        assert split is not None
        assert splitter.stats.split == 1
        assert splitter.stats.probes == split.probes

    def test_a_refused_review_is_counted(self):
        splitter = LanguageSplitter(FixedProber(
            decided("vi"), decided("ja"), decided("vi"), decided("vi"),
            decided("vi"), decided("vi"), decided("vi")))
        splitter.find(audio(8.0))
        assert splitter.stats.split == 0
        assert (splitter.stats.refused_on_review
                + splitter.stats.sliver) == 1

    def test_the_hangover_is_passed_through(self):
        """A length cut has none, so a tail may sit closer to the end."""
        pcm = two_turns(4.0, 1.4)
        assert LanguageSplitter(TimedProber("vi", "ja")).find(
            pcm, hangover_ms=500) is None
        assert LanguageSplitter(TimedProber("vi", "ja")).find(
            pcm, hangover_ms=0) is not None

    def test_reset_forgets_the_counts(self):
        splitter = LanguageSplitter(FixedProber(decided("vi")))
        splitter.find(audio(6.0))
        splitter.reset()
        assert splitter.stats.checked == 0
