...
class TestStability:
    """What a reader notices first: how much of the line they were reading
    the next update kept."""

    def test_an_update_that_extends_the_line_keeps_all_of_it(self):
        assert debuglog.survival("xin chào", "xin chào mọi người") == 1.0

    def test_an_update_that_rewrites_the_start_keeps_nothing(self):
        assert debuglog.survival("xin chào", "chào anh") == 0.0

    def test_an_empty_line_has_nothing_to_lose(self):
        assert debuglog.survival("", "xin chào") == 1.0

    def test_the_run_reports_the_mean_and_the_wipes(self, run):
        kept, wiped = debuglog.stability(run)
        # Sentence 1: 画面共有しましたが -> はい keeps nothing.
        # Sentence 2: Tạp cấp... -> Về mục tiêu... keeps nothing either.
        assert kept == 0.0
        assert wiped == 1.0
        found = debuglog.measure(run)
        assert found["update_survival"] == 0.0
        assert found["updates_wiping_half"] == 1.0


def test_the_comparison_table_names_a_different_meeting(tmp_path):
    from server.analysis import compare_logs

    first = tmp_path / "a.debug.txt"
    first.write_text(LOG, encoding="utf-8")
    other = tmp_path / "b.debug.txt"
    other.write_text(
        "00:00:00.000      0.0s  start       session=?\n"
        "00:00:05.000      5.0s  final       #1 Speaker_01 [vi] "
        "Một cuộc họp hoàn toàn khác\n", encoding="utf-8")
