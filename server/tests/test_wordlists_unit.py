...
def test_the_prompt_holds_only_the_confirmed_words():
    ...
    # Twelve terms seen misheard, each with what it was heard as, and five
    # names the meeting supplied.
    assert len(words) <= 20, f"{len(words)} words is a stuffed prompt"
...
def test_only_the_vocabulary_file_feeds_the_prompt():
    ...
    # The prompt the decoder is handed comes from one place.
    assignment = next(row for row in source.splitlines()
                      if "else vocabulary_prompt()" in row)
    assert "hallucination" not in assignment.lower()
    assert source.count("vocabulary_prompt()") == 1


def test_a_trailing_note_is_cut_where_it_opens_with_a_space(tmp_path,
                                                           monkeypatch):
    from server import wordlists as module

    (tmp_path / "vocabulary.txt").write_text(
        "Slack   # heard as quạt nắp\nC#\nSlack\n", encoding="utf-8")
    monkeypatch.setattr(module, "DATA_DIR", tmp_path)
    assert module.vocabulary_terms() == ["Slack", "C#"]


def test_the_prompt_is_cut_on_a_term_boundary(tmp_path, monkeypatch):
    from server import wordlists as module
