...
| `vocabulary.txt` | **Từ vựng mồi** cho Whisper (`initial_prompt`) — không chặn gì |

Đổi chỗ đặt bằng biến môi trường `MEETING_DATA_DIR`.

Danh sách chặn/giữ **không bao giờ tới Whisper**: chúng được so với chữ Whisper
đã viết ra. Chỉ `vocabulary.txt` thành `initial_prompt`, và chỉ câu chốt nhận
nó. Hai quy tắc của file đó nằm ngay đầu file.
